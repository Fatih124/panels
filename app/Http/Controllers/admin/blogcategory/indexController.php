...
class indexController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        if ($c!=0)
        {
            ...
            return view('admin.blogcategory.edit', ['data'=>$data, 'id'=>$id]);
        }
        else
        {
            return abort(404);
    ...
    public function store(Request $request)
    {
        ...
        if ($create) {
            ...
            if (isset($all['image'])){
                LanguageContent::InsertorUpdate($all['image'], BLOG_CATEGORY_LANGUAGE, IMAGE_LANGUAGE, $create->id, 1, "services");
            }
    ...
    {
        $query = BlogCategory::query();//query olusturlur verıler alınır
        ...
        ->addColumn('name', function ($query){
            ...
            ->addColumn('edit', function ($query){//datatable kolon olarak eklenir
                return '<a href="'.route('admin.blogcategory.edit',['id'=>$query->id]).'">Düzenle</a>';//buton basılır
            })
            ->addColumn('delete', function ($query){
                return '<a href="'.route('admin.blogcategory.delete',['id'=>$query->id]).'">Sil</a>';//silme butonu basılır
            })
